...
function addItem(){
    var textval = document.getElementById("item").value;
    if (textval) {  
        addItemTodo(textval);
        document.getElementById('item').value = '';
        data.current.push(textval); 
        dataUpdated();
    }
}
...
function removeItem(){
    ...
    if(list.id == 'current') {
        data.current.splice(data.current.indexOf(item),1);
    } else {
...
function movetoSprint(){
    ...
    var list = item.parentNode;
    var id = list.id;
    var target = id =='current'? document.getElementById('completed'): document.getElementById('current');
    list.removeChild(item);
...
function addItemTodo(text, completed) {
    var list = (completed) ? document.getElementById('completed'):document.getElementById('current');
    ...
    complete.addEventListener('click', movetoSprint);
...
function renderTodoList() {
    if(!data.current.length && !data.completed.length)  {
        return;
    } else {
